<template>
  <div class="org-page" :class="{ 'light-theme': isLightTheme }">
    ...
    <div class="content-wrapper">
      ...
      <div class="team-page">
        ...
        <div class="page-header">
          ...
          <!-- Dev-only manage button -->
          <button v-if="isDev" class="dev-manage-btn" @click="showManager = true">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
        ...
        <div class="glass-card">
          ...
          <div class="chart">
            ...
            <div class="luxury-container" v-else>
              <template v-for="(levelGroup, li) in processedHierarchy" :key="li">
                <div class="level-group-wrapper">
                  ...
                  <div class="category-tag">
                    ...
                    <div class="tag-content-block">
                      <div class="tag-primary">
                        ...
                        <span class="tag-label" :data-text="levelGroup.roleName">{{ levelGroup.roleName }}</span>
                      </div>
                      
                      <div class="tag-secondary">
                        <div class="status-pulse"></div>
                        <span class="tag-meta">// DATALINK_ESTABLISHED: [{{ levelGroup.members.length }}] UNITS_ONLINE</span>
                      </div>
                    </div>
                  ...
                  <!-- Empty Role Message -->
                  <div v-if="levelGroup.rows.length === 0" style="padding: 15px 25px; color: rgba(255,255,255,0.4); font-style: italic; font-size: 0.9em;">
                    There are currently no staff members holding this rank.
                  </div>

                  <template v-for="(row, ri) in levelGroup.rows" :key="'r-' + li + '-' + ri">
                    <div class="luxury-grid">
                      <div v-for="m in row" :key="m.id" class="glass-card-wrapper">
                        <div class="discord-glass-card" :class="levelGroup.roleClass">
                          
                          <!-- Banner superior Liquid -->
                          <div class="luxury-banner" :style="{ backgroundColor: levelGroup.color, backgroundImage: bannerData[m.id] ? `url(${bannerData[m.id]})` : 'none' }">
                          ...
                          <div class="luxury-content">
                            ...
                            <div class="avatar-anchor">
                              <div class="avatar-frame" :style="{ borderColor: levelGroup.color }">
                                <img v-if="memberData[m.id]" :src="memberData[m.id]" class="img-fluid" @error="handleAvatarError(m.id)" />
                                <div v-else class="init-fluid" :style="{ background: levelGroup.color }">{{ m.init }}</div>
                              </div>
                            ...
                            <div class="luxury-info">
                              ...
                              <div class="role-display">
                                ...
                                <div class="role-label" :style="{ '--role-color-static': levelGroup.color }">
                                  <span class="dot" :style="{ color: levelGroup.color }"></span>
                                  <span class="text" :style="{ color: levelGroup.color }">{{ levelGroup.roleName }}</span>
                                </div>
                              </div>
                            </div>
                            
                            <!-- Bio Section (Smooth Reveal) -->
                            <div class="card-bio" v-if="m.bio || levelGroup.roleBio">
                              <p class="bio-text">{{ m.bio || levelGroup.roleBio }}</p>
                            </div>
    ...
    <!-- Team Manager Modal (Dev Only) -->
    <TeamManager 
      v-if="showManager" 
      :modelValue="hierarchyDefinition" 
      @close="showManager = false" 
      @copied="showManager = false"
    />
  </div>
</template>
...
const totalStaffCount = computed(() => {
  return hierarchy.value.map(l => l.members || []).flat().length
})
  ...
  },
  {
    ...
    members: [
      { 
        ...
        name: 'langw0w', 
        display: 'langw0w', 
        fullRole: 'Community Advisor', 
        emoji: '🐦‍🔥', 
        discordId: '426490596950147074', 
        color: '#c4511a', 
        roleClass: 'comm.advisor', 
        cls: 'glow-admin', 
        init: 'L', 
...
// 3D effect removed as requested
function goBack() {
  window.history.back()
}

onMounted(async () => {
  // Setup DEV tools if running locally
  isDev.value = window.location.hostname === 'localhost' || import.meta.env.DEV

  const allMembers = hierarchy.value.map(l => l.members || []).flat()
...
.light-theme .stats-label { color: rgba(0, 0, 0, 0.5); }
...
.light-theme .stats-value { color: #111; }
...
.light-theme .stats-divider-v { background: rgba(0, 0, 0, 0.12); }
...
.light-theme .tag-meta { color: rgba(0, 0, 0, 0.5); }
...
.light-theme .discord-glass-card {
...
.light-theme .discord-glass-card:hover {
...
.discord-glass-card:hover .shimmer-effect {
  left: 150%;
...
.avatar-frame {
  ...
  height: 100%;
  border-radius: 50%;
  border: 4px solid rgba(10, 10, 12, 0.8);
  background: #111;
  overflow: hidden;
  position: relative;
...
.online-status {
  ...
  background: #23a55a;
  border: 3px solid rgba(10, 10, 12, 0.8);
  border-radius: 50%;
}

.light-theme .online-status {
  border-color: rgba(255, 255, 255, 0.8);
}
...
.light-theme .handle { color: rgba(0,0,0,0.5); }
...
.light-theme .tiny-title { color: rgba(0,0,0,0.5); }
...
.light-theme .card-bio {
  background: rgba(0, 0, 0, 0.03);
}

.discord-glass-card:hover .card-bio {
  opacity: 1;
  max-height: 200px;
...
.light-theme .btn-copy-glass {
  background: rgba(255, 255, 255, 0.4);
  border-color: rgba(0, 0, 0, 0.1);
  color: rgba(0, 0, 0, 0.7);
}

.light-theme .btn-copy-glass:hover {
  background: rgba(255, 255, 255, 0.8);
  color: #000;
}

/* ============================================================
   MODAL ADMIN (TEAM MANAGER)
   ============================================================ */
.dev-manage-btn {
  background: rgba(255, 120, 0, 0.1);
