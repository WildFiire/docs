...
onMounted(async () => {
  ...
  // Optimization: Pause the WebGL shader when it scrolls out of view
  if (typeof IntersectionObserver !== 'undefined') {
    const observer = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (shaderMount) {
          if (entry.isIntersecting) {
            shaderMount.paused = false
          } else {
            shaderMount.paused = true
          }
        }
      })
    }, { rootMargin: '100px' })
    ...
    shaderMount._observer = observer
  }
...
onUnmounted(() => {
  if (shaderMount) {
    if (shaderMount._observer) shaderMount._observer.disconnect()
    shaderMount.dispose()
    shaderMount = null
  }
})
